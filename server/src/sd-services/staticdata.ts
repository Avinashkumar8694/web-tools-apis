...
export class staticdata {
  ...
  async mountTimers() {
    try {
      //appendnew_flow_staticdata_TimerStart
    } catch (e) {
      throw e;
    }
  }
  ...
  private mountAllMiddlewares() {
    log.debug('mounting all middlewares for service :: staticdata');

    //appendnew_flow_staticdata_MiddlewareStart
  }
  private mountAllPaths() {
    log.debug('mounting all paths for service :: staticdata');
    //appendnew_flow_staticdata_HttpIn
  }
  //   service flows_staticdata

  async getGoogleDomain(...others) {
    try {
      var bh: any = {
        input: {},
        local: {
      ...
      bh = await this.sd_Jl5uF30CJAmDwRrv(bh);
      //appendnew_next_getGoogleDomain
      return (
      ...
      return await this.errorHandler(bh, e, 'sd_8hsmsTc8jtnv53qZ');
    }
  }

  async randomAgent(...others) {
    try {
      var bh: any = {
        input: {},
        local: {
      ...
      bh = this.sdService.__constructDefault(bh);
      bh = await this.sd_iiPCWGvN09KVDrZM(bh);
      //appendnew_next_randomAgent
      return (
        // formatting output variables
        {
          input: {},
      ...
      return await this.errorHandler(bh, e, 'sd_Ev4IMCpys87dPWnQ');
    }
  }

  //appendnew_flow_staticdata_start

  async sd_Jl5uF30CJAmDwRrv(bh) {
    try {
      bh.googleDomains = [
        'google.com',
        'google.ad',
      ...
      bh.local.response =
        bh.googleDomains[Math.floor(Math.random() * bh.googleDomains.length)];
      //appendnew_next_sd_Jl5uF30CJAmDwRrv
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_Jl5uF30CJAmDwRrv');
    }
  }

  async sd_iiPCWGvN09KVDrZM(bh) {
    try {
      bh.agentArray = [
      ...
      bh = await this.sd_CjfMYLsxt3Mn5eQN(bh);
      //appendnew_next_sd_iiPCWGvN09KVDrZM
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_iiPCWGvN09KVDrZM');
    }
  }

  async sd_CjfMYLsxt3Mn5eQN(bh) {
    try {
      bh.local.response = {
        'User-Agent':
          bh.agentArray[Math.floor(Math.random() * bh.agentArray.length)],
      };
      //appendnew_next_sd_CjfMYLsxt3Mn5eQN
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_CjfMYLsxt3Mn5eQN');
    }
  }
